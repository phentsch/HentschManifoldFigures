...
elevation = 26 # elevation angle in degrees
azimuth = -22 # azimuth angle in degrees
...
# --- Additional Plots ------------------------------------------------
ax.set_title(r'$Q(\alpha, \beta, \gamma) = 0$', fontsize=title_size, fontstyle=title_style, fontfamily=title_family, loc=title_location, pad=title_padding)
...
# --- Rotated axes: α, β, γ as double-ended lines through the origin -------
axis_length = 1.5  # extend beyond unit cube
alpha = rotated_vectors[:, 0] * axis_length
beta = rotated_vectors[:, 1] * axis_length
gamma = rotated_vectors[:, 2] * axis_length

ax.plot([-alpha[0], alpha[0]], [-alpha[1], alpha[1]], [-alpha[2], alpha[2]], color='magenta', linestyle=':', linewidth=1)
ax.text(*(alpha * 1.1), r'$\alpha$', fontsize=6, color='magenta')
ax.text(*(-alpha * 1.1), r'$-\alpha$', fontsize=6, color='magenta')

ax.plot([-beta[0], beta[0]], [-beta[1], beta[1]], [-beta[2], beta[2]], color='#eeee00', linestyle=':', linewidth=1)
ax.text(*(beta * 1.1), r'$\beta$', fontsize=6, color='#eeee00')
ax.text(*(-beta * 1.1), r'$-\beta$', fontsize=6, color='#eeee00')

ax.plot([-gamma[0], gamma[0]], [-gamma[1], gamma[1]], [-gamma[2], gamma[2]], color='cyan', linestyle=':', linewidth=1)
...
# Arrow v₁ = (1, 0, 0)
ax.quiver(0, 0, ell_value, 0, 1, 0, color=label_color, arrow_length_ratio=0.1, linewidth=1)
ax.text(0, 1.1, ell_value, r'$\vec{v}_1$', fontsize=8, color=label_color)

# Arrow v₂ = (0, 1, 0)
ax.quiver(0, 0, ell_value, -1.1, 0, 0, color=label_color, arrow_length_ratio=0.1, linewidth=1)
ax.text(-1.1, 0, ell_value, r'$\vec{v}_2$', fontsize=8, color=label_color)

# Radius
ax.quiver(0, 0, ell_value, 0, ell_radius, 0, color='k', arrow_length_ratio=0, linewidth=1) # radius
ax.text(0, 0.25*ell_radius, ell_value+0.075, r'$r = \sqrt{2}\, \ell$', fontsize=8, color='k') # radius label
ax.quiver(0, 0, 0, 0, ell_radius, ell_value, color='k', arrow_length_ratio=0, linewidth=1) # hypotenuse
ax.quiver(0, 0, 0, 0, 0, ell_value, color='k', arrow_length_ratio=0, linewidth=3) # adjacent
ax.text(0, 0.075, 0.15*ell_value, r'$\theta = \tan^{-1}(\sqrt{2})$', fontsize=8, color='k') # angle label

# --- Add screen plane orthogonal to ℓ = (1,1,1) -------------------------------
# Define horizontal screen plane at z = 0.5 (orthogonal to ℓ aligned to z-axis)

ell_value2 = -0.5
ell_radius2 = np.sqrt(2) * ell_value2
screen_range = np.linspace(-1, 1, 2)
X_screen, Y_screen = np.meshgrid(screen_range, screen_range)
Z_screen = np.full_like(X_screen, ell_value2)
...
screen_color = cmap(norm(ell_value2))
ax.plot_surface(X_screen, Y_screen, Z_screen, alpha=0.2, color=screen_color, edgecolor='none')
ax.text(-0.96, -0.96, ell_value2, r'$\ell^\perp$', fontsize=10, color=screen_color)

# Add a circular null cone cross-section on the screen plane
circle_radius = np.sqrt(2)*(ell_value2)  # approximate radius; adjust as needed
theta = np.linspace(0, 2 * np.pi, 100)
x_circle = circle_radius * np.cos(theta)
y_circle = circle_radius * np.sin(theta)
z_circle = np.full_like(x_circle, ell_value2)
...
val_label = ell_value2
label_color = cmap(norm(val_label))
#--- ax.text(0, -1.6, ell_value2, r'$\mathcal{S}_P = T_P\mathcal{M}_{\text{H}} / \text{span}(\ell)$', fontsize=8, color=label_color)
#--- ax.text(0.15, 0.15, ell_value2, 'sub-null', fontfamily='serif', fontstyle='italic', fontsize=7, color=label_color)
#--- ax.text(0.65, 0.65, ell_value2, 'super-null', fontfamily='serif', fontstyle='italic', fontsize=7, color=label_color)


# --- Additional arrows: v₁ = (1,0,0), v₂ = (0,1,0) ---------------------------
# Arrow v₁ = (1, 0, 0)
ax.quiver(0, 0, ell_value2, 0, 1, 0, color=label_color, arrow_length_ratio=0.1, linewidth=1)
ax.text(0, 1.1, ell_value2, r'$\vec{v}_1$', fontsize=8, color=label_color)

# Arrow v₂ = (0, 1, 0)
ax.quiver(0, 0, ell_value2, -1.1, 0, 0, color=label_color, arrow_length_ratio=0.1, linewidth=1)
ax.text(-1.1, 0, ell_value2, r'$\vec{v}_2$', fontsize=8, color=label_color)

# Radius
ax.quiver(0, 0, ell_value2, 0, ell_radius2, 0, color='k', arrow_length_ratio=0, linewidth=1) # radius
ax.text(0, 0.7*ell_radius2, ell_value2+0.075, r'$r = \sqrt{2}\, \ell$', fontsize=8, color='k') # radius label
ax.quiver(0, 0, 0, 0, ell_radius2, ell_value2, color='k', arrow_length_ratio=0, linewidth=1) # hypotenuse
ax.quiver(0, 0, 0, 0, 0, ell_value2, color='k', arrow_length_ratio=0, linewidth=3) # adjacent
...
plt.show()
# Save the figure
# fig.savefig('fig_quadratic_null_cone_2a.png', dpi=figure_dpi, bbox_inches='tight')
